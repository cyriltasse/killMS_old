...
class ClassPierce():
    ...
    def givePP(self,time,rain,decin,asel=[],giveAir=False,reshape=False):
        ...
        p = me.position('ITRF', '%fm' % self.antenna_positions[0,0], '%fm' % self.antenna_positions[0,1], '%fm' % self.antenna_positions[0,2])
        me.doframe(p)



        epoch = me.epoch('UTC', '%fs' % time)
        me.doframe(epoch)
        pp_idx = 0
        import ClassTimeIt
        ...
        for i in range(ra.shape[0]):
            d = me.direction('J2000', '%frad' % ra[i], '%frad' % dec[i])
