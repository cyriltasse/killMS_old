...
def PlotTessel(xin,yin,extent=None):
    import pylab
    x=xin.flatten()
    y=yin.flatten()
    Ncells=400
...
def tessel(xin,yin,sin,NCluster,DoPlot=True):
    ...
    buildBorder(im,DictNode,ax=0)
    buildBorder(im,DictNode,ax=1)
    ...
    while True:
        ...
        DictNode=mergeOne(DictNode,ListDictNode[iii][0])
...
def buildBorder(im,DictNode,ax=0):
    if ax==0:
