...
class ClassMakeTOP():
    ...
    def make_A0new(self,row0,row1):
        ...
        A0mat=np.zeros((NSPWChan*Ntimes*na*na,NDir*na*2),dtype=np.complex)
        Amat=np.zeros((NSPWChan*Ntimes*na,NDir*na*2),dtype=np.complex)
        ...
        for i in range(self.SM.NDir):
            ...
            for spw in range(NSPWChan):
                ...
                for bl in range(nbl):
                    ...
                    icoord=a0*na*Ntimes+a1*Ntimes
                    jcoord=i+a1*NDir
                    A0mat[icoord:icoord+Ntimes,jcoord]=vv[bl::nbl]

                    icoord=a1*na*Ntimes+a0*Ntimes
                    jcoord=i+a0*NDir
                ...
                for bl in range(nbl):
                    ...
                    icoord=a0*na*Ntimes+a1*Ntimes
                    jcoord=i+a0*NDir+na*NDir
                    A0mat[icoord:icoord+Ntimes,jcoord]=vv[bl::nbl]

                    icoord=a1*na*Ntimes+a0*Ntimes
                    jcoord=i+a1*NDir+na*NDir
                    A0mat[icoord:icoord+Ntimes,jcoord]=vv[bl::nbl].conj()
    ...
    def make_A0new_ReIm(self,row0,row1):
        ant0=self.MS.A0[row0:row1]
        ant1=self.MS.A1[row0:row1]
        Ntimes=(row1-row0)/self.MS.nbl
        na=self.MS.na
        ...
        for i in range(self.SM.NDir):
            ...
            for spw in range(NSPWChan):
                ...
                for bl in range(nbl):
                    ...
                    icoord=a0*na*Ntimes+a1*Ntimes
                    jcoord=i+a1*NDir+na*NDir
                    A0mat[icoord:icoord+Ntimes,jcoord]=1j*vv[bl::nbl]
                    ...
                    icoord=a1*na*Ntimes+a0*Ntimes
                    jcoord=i+a0*NDir+na*NDir
                    A0mat[icoord:icoord+Ntimes,jcoord]=1j*vv[bl::nbl].conj()
